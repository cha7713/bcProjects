...
/*
 - 구조
   function 함수명 ( 타입 x1, 타입 x2, ... ) options returns ( 타입 x1(생략가능), 타입 x2, ... )
   {
      // satement; ...
      return ( ... )   // <- 생략 가능
   }
 - options
  1) 가시성    : (access modifier)
      ...
      - internal : 해당 컨트렉트와 상속 컨트렉트 에만 공개
      - private  : 오직, 해당 컨트렉트만 공개
  2) 비용측면 : 
      - constant(X) (더이상사용되지 않는다 정의) -> 이렇게 정의된 함수는 gas를 소모하지 않는다
      - view : 다음 행위를 함수 내부에서 하지 않겟다 <- 아래 코드를 해야 하면 view사용 불가
                 1) 상태 변수 값 변경
                 2) 이벤트 발생 (이벤트 -> 컨트렉트 -> nodejs쪽으로 이벤트 전달)
                 3) 다른 컨트렉트 생성
                 4) 이더 전송(송금)
                 5)  view나 pure로 선언되지 않은 함수 호출
      - pure : 다음 행위를 함수 내부에서 하지 않겟다 <- 아래 코드를 해야 하면 pure 사용 불가                              1) 상태 변수를 읽는다 
                 1) 상태 변수를 읽는다 
                 2) xx.balance 잔액확인등 접근시도
                 3) 컨트렉트이 사전에 이미 정의된 전역객체들 block, tx, msg등을 접근행위
                 4) pure로 정의되어 있지 않는 함수를 호출
      - payable : 비용이 발생된다. 돈이 움직이다
                 ex) 컨트렉트가 맴버 함수 를 통해서 다른 지갑(타인), 이더를 송금하거나 받는등
...
contract BasicFunc
{
    ...
    // t4 함수를 구현하는데 부호없는 정수 2개를 입력받아서 더한값을 리턴하는 함수를 구형하시오
    function t4 (uint x, uint y) public pure returns (uint)
    {
        uint result = x + y;
        return result;
    }
