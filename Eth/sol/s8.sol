...
contract BasicString
{
    ...
    public pure returns (string memory)
    {
        ...
        // 문자열변환, 모든데이터는 bytes에 담겼으므로, string 변환
    ...
    // 문자열 더하기
    // addString
    // input:  string, string => string->bytes변환 2개
    // output: string
    ...
    public pure returns (string memory)
    {
        // 2. input string->bytes변환 2개, s1, s2 variable name
    ...
    // output: string
    function subString (string memory src, uint offset, uint size)
    public pure returns (string memory)
    {
        // string -> bytes
        bytes memory tmp = bytes( src );
