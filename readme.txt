...
9. 공정관리
 ...
 - 설치 -> choosing editor xx 메뉴에서 visual studio code 첫번째것으로 선택 -> 끝까지 진행
 - new > 새로운 레퍼지토리 생성 > 만약 private 생성 권한(기본은 유료)이 있으면 
  사용(택일)
 - 레파지토리 메인 화면에서 > Clone or Download > 주소 카피
 ...
 ------------------------------------------------------------
 현 프로젝트 공간에서 사용자 등록이 않되면 실패 1회만
 ouptut 창에 해당 내용이 나와 있음
 $ git config --global user.name "계정이름"
 $ git config --global user.email "계정이메일"
 하단 부분에 push/pull 아이콘 클릭해서 동기화 처리 수행
 ------------------------------------------------------------
 작업 동선에 따라, push와 pull을 겹치치 않게 잘 처리한다
